 ...
 * @author alos
 */
public class VolumeDescriptor extends Descriptor{
    ...
    DescriptorAnswer resp = new DescriptorAnswer();
        ...
        for (Statement s : this.statements) {
            ...
            for (Token tok : s.getStatement()) {
                ...
                /*For INTEGER tokens*/
                if (tok.getTokenType().equals(TokenType.INTEGER)) {
                    boolean isTokenInteger = false;
                    String aPossibleInteger = "";
                    try {
                        ...
                        for (;; i++) {
                            if (aString.length() > i) {
                                ...
                                if (Character.isDigit(c)) {
                                    aPossibleInteger = aPossibleInteger + c;
                                } else {
                                    isTokenInteger=false;
                                    break;
                                }
                            }else
                                break;
                    ...
                    if(s.statement.size()==1){
                        if (aPossibleInteger.length() == aString.length() && aString.length() < 4)//and some other stuff i can think of
                            isTokenInteger = true;
                    }else{
                         if (aPossibleInteger.length() > 0)//and some other stuff i can think of
                            isTokenInteger = true;
                    }
                        ...
                        if (tok.getToken() != null) {
                            if (tok.getToken().length() == 1) {
                                ...
                                if (tok.getToken().length() != 0) {//it means we got a word to compare
                                    ...
                                    for (; i < last; i++) {
                                        if(aString.charAt(i) == tok.getToken().charAt(i)){
                                            isTokenString = true;
                                        }else{
                                            isTokenString=false;
                                            break;
        ...
        resp.setDescriptorType(DescriptorType.VOLUME);
        resp.addToScore(10);
        Volume vol = new Volume(aString);
        resp.setObject(vol);
        return resp;
    }
